...
Team profiles must be regular files, not symbolic links. The CLI reads a profile once, validates its fixed frontmatter against the built-in department and skill registries, and retains four normalized fields. It sends only scope, departments and skills to an engine prompt. The free-form body and stored research status stay local, and stored research metadata never authorizes network access. This prevents profile body instructions and later file replacement from crossing the prompt boundary.
